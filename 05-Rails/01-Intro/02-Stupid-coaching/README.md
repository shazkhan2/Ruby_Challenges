...
If not, go back to the dedicated section of the run `gem install rails`
...
There is no `rake` here. Also, do not create your Rails app in `karnov-academy-challenges`.
...
Last step to display the form, let's create a view! Do you remember in which folder it should be and how it should be named? That's one of Rails' conventions, the _Action View convention_. Refresh the page at [localhost:3000/ask](http://localhost:3000/ask), if you named your file properly you should finally see a page without an error! For now it's empty, let's finally add the `<form>`. Remember the syntax?
