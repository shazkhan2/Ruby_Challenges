...
Time to implement a "Longest-word game" with a cool web-interface!

Before jumping into the exercise, here are the rules:
...
There is no `rake` here, and do not create your Rails app in `karnov-academy-challenges`. You should be creating your app in the following directory:
...
How do you generate an `Array` of ten random letters? In the `new` action of the `GamesController`, create a new `@letters` instance variable storing these random letters from the alphabet. Then display it in the view. You should get something like this:
