def valid_section?(section)
  ...
  valid_section = ->(section) { section.uniq.size == 9 }
  valid_section.call(section)
end
...
# def valid_sudoku?(board)
  ...
  def valid_sudoku?(board)
    valid_section = ->(section) { section.uniq.size == section.size }
  
    board.all?(&valid_section) &&                  
      board.transpose.all?(&valid_section) &&     
      regions(board).all?(&valid_section)          
  end
